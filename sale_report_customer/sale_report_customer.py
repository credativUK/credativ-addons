...
class sale_report_customer_base(object):
    def _suppliers_get(self, partner):
        ...
        res = {}
        where_clauses = ["so.state NOT IN ('draft', 'cancel')", "cust.id = %s"]
        where_args = [(), (partner,)]
        if date_from:
        ...
        self.cr.execute("""WITH RECURSIVE bom_relation(bom, parent, qty) AS (
                        ...
                        ) SELECT cust.id, supp.id, so.date_order, pp.id, SUM(sol.product_uom_qty * COALESCE(br.qty, 1.0)), sol.product_uom
                        FROM sale_order so
                        ...
                        LEFT OUTER JOIN res_partner supp ON ds.partner_id = supp.id
                        %s
                        GROUP BY cust.id, supp.id, so.date_order, pp.id, sol.product_uom
                        ORDER BY cust.id, supp.id, so.date_order, pp.id
                   """ % where_string)
...
class sale_report_customer(report_sxw.rml_parse, sale_report_customer_base):
    def __init__(self, cr, uid, name, context):
        super(sale_report_customer, self).__init__(cr, uid, name, context=context)
        self.localcontext.update( {
            'time': time,
            'adr_get': self._adr_get,
            'getSupplierData': self._suppliers_get,
            'getDateText': self._date_text_get,
        })
        self.context = context

report_sxw.report_sxw('report.sale.customer.pdf', 'res.partner', 'addons/sale_report_customer/sale_report_customer.rml', parser=sale_report_customer)
...
class sale_report_customer_csv_base(sale_report_customer_base):
    cr = None
    uid = None
    context = None
...
def sale_report_customer_csv(cr, uid, ids, data, context):
    row = ['Customer Ref',
           'Customer',
           'Supplier Ref',
           'Supplier',
    ...
    report_base = sale_report_customer_csv_base(cr, uid, context)
    ...
    for customer in res_partner.browse(cr, uid, ids, context=context):
        for supplier, supplierdata in report_base._suppliers_get(customer.id).iteritems():
            for uom, data in supplierdata.iteritems():
                for datarow in data:
                    row = [customer.ref,            # Customer Ref
...
csv_register('report.sale.customer.csv', sale_report_customer_csv)
